...
PRODUCT_SELECTION_PROMPT=f"Given the goal, context, the key points of each product, and the details of the resource provided, select {4} products that are most necessary for a chatgpt to achieve the goal."
...
class Agent:
    def __init__(self,product_type,products,api_key,log_file='./log/chatgpt.log',product_detail_path='./data/summary.json',log_level=logging.DEBUG):
        ...
        self.product_name=self.products['title'].tolist()
        ...
        load=False
        if os.path.exists(product_detail_path):
            with open(product_detail_path,'r') as f:
                self.product_detail=json.load(f)
            if len(self.product_detail)==len(self.products):
                load=True
        ...
        if load==False:
            ...
            with open(product_detail_path,'w') as f:
                json.dump(self.product_detail,f)
    ...
    def json_chat(self,sys_message,schema_name,user_message,concerened_key=None,strict_mode=True,**kwargs):
        if user_message!='':
        ...
        for i in range(5):
            if assistant_reply_json['success']==True or strict_mode==False:
                break
            message_send=message+[{"role":"system","content":f"Your response ```{assistant_reply_json['val']} ``` cannot be parsed using `ast.literal_eval` because {assistant_reply_json['error']} \n\n Please respond with only valid JSON conforming to the following schema: \n{llm_response_schema(schema_name)}\n"}]
    ...
    def main_loop(self):
        while True:
            ...
            print(f'Goal:{goal}')
            instruction=self.json_chat(f"{self.SYSTEM_PROMPT}\n{ACHIEVE_PROMPT} {goal}? Please describe as short as possible",'instruction','','goal_instruction',strict_mode=False,max_tokens=50)
            ...
            if required_resource['need_user_preference']==True: # need user's specified information
                ...
                if required_resource['need_product_details']==True:
                    selected_products=self.json_chat(f"{self.SYSTEM_PROMPT}\n{PRODUCT_SELECTION_PROMPT}\nGoal:{goal}\nInstruction:{instruction}\nProduct Key Features:{';'.join(self.product_detail)}\nResources:{resources}\nContext: {context_string}",'product_select','')['Necessary_Products']
                    print(selected_products)
                    import fuzzywuzzy as fz
                    from fuzzywuzzy import process
                    product_string=''
                    for p in selected_products.keys():
                        best_match = process.extractOne(selected_products[p], self.product_name)
                        idx=self.product_name.index(best_match[0])
                        product_string+=f"\n{p}:{self.product_info.iloc[idx]}"
